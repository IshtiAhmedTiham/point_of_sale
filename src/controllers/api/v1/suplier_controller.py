...
from src.schemas.suplier_schema import (
    CreateSuplier, 
    ResponseSuplier, 
    create_suplier_form
)
...
def create_suplier(
    data : CreateSuplier = Depends(validate_suplier), 
...
def read_suplier(
    filters : Annotated[SuplierFilters, Query()] = None, 
    db : Session = Depends(get_db)
):
    suplier = (
        db.query(SuplierModel)
        .filter(SuplierModel.deleted_at.is_(None))
    )
